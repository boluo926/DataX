...
<br />
    ...
    **注意**：
    ...
    3. `MERGE INTO` 支持批量操作，性能约为 insert 模式的 50%-70%
...
* **writeMode**
    ...
    **注意**：
    ...
    - replace 模式：使用 MERGE INTO，主键冲突时更新，否则插入，支持批量操作
    - update 模式：与 replace 相同，使用 MERGE INTO，支持批量操作
...
2. **批量操作**：
   ...
   - REPLACE/UPDATE 模式也支持批量操作，性能约为 INSERT 模式的 50%-70%
...
**Q: 为什么 replace/update 模式比 insert 模式慢一些？**

A: 因为达梦数据库的 replace/update 模式使用 MERGE INTO 语法实现，该语法虽然支持批量操作，但需要额外的主键匹配和条件判断，因此性能约为 insert 模式的 50%-70%。如果不需要处理主键冲突，建议使用 insert 模式以获得最佳性能。
